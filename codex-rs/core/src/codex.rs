...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = async {
        loop {
            ...
            let event = tokio::select! {
                biased;
                tool_output = in_flight.next(), if !in_flight.is_empty() => {
                    match tool_output {
                        Some(Ok(response_input)) => {
                            sess.record_conversation_items(&turn_context, &[response_input.into()])
                                .await;
                        }
                        Some(Err(err)) => {
                            error_or_panic(format!(
                                "in-flight tool future failed during stream handling: {err}"
                            ));
                        }
                        None => {}
                    }
            ...
            match event {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    ...
                    let mut ctx = HandleOutputCtx {
                        ...
                        cancellation_token: tool_cancellation_token.child_token(),
                    };
